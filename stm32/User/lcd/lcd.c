...
static u8 LCD_pointer = 0;
static char LCD_buffer[2][MAX_CHAR_IN_COLUMN][MAX_CHAR_IN_PAGE];
...
void LCD_INIT ( void )
{
	...
	LCD_OpenWindow ( 0, 0, LCD_Default_Max_COLUMN, LCD_Default_Max_PAGE );
	LCD_FillColor ( LCD_Default_Max_COLUMN * LCD_Default_Max_PAGE, BACKGROUND);	
	
	for (u8 i=0; i<MAX_CHAR_IN_COLUMN; i++){
		for (u8 j=0; j<MAX_CHAR_IN_PAGE; j++){
			LCD_buffer[0][i][j] = 0;
			LCD_buffer[1][i][j] = 0;
		}
	}
...
inline void LCD_DrawChar ( uint16_t usC, uint16_t usP, const char cChar )
{
...
void LCD_Update(){
	for (u8 i=0; i<MAX_CHAR_IN_COLUMN; i++){
		for (u8 j=0; j<MAX_CHAR_IN_PAGE; j++){
			if (LCD_buffer[0][i][j] != LCD_buffer[1][i][j]){
				LCD_DrawChar(i*WIDTH_EN_CHAR, j*HEIGHT_EN_CHAR, LCD_buffer[LCD_pointer][i][j]);
			}
		}
	}
	
	LCD_pointer = (LCD_pointer+1) % 2;
}

void LCD_Clear(){
	memset(LCD_buffer[LCD_pointer], 0, MAX_CHAR_IN_COLUMN*MAX_CHAR_IN_PAGE);
}

void LCD_DrawString ( uint16_t x, uint16_t y, const char * pStr )
{
	while ( * pStr != '\0' )
	{
		if ( x > MAX_CHAR_IN_PAGE )
		{
			x = 0;
			y += 1;
		}
		
		if (y > MAX_CHAR_IN_COLUMN )
		{
			x = 0;
			y = 0;
		}
		
		LCD_buffer[LCD_pointer][x][y] = *pStr - ' ';
...
{	
	/*
	 *  Task 2 ֠Implement the LCD_DrawDot to turn on a particular dot on the LCD.
	 */
	LCD_OpenWindow(usCOLUMN, usPAGE, 1, 1);
	LCD_Write_Cmd ( CMD_SetPixel );	
...
static int16_t my_abs(int16_t v){
...
{
	/*
	 *  Task 3 ֠Implement LCD_DrawCircle by using LCD_DrawDot
	 */
	uint16_t R_squared = R*R;
	int16_t i = -R;
	int16_t width = 0;
	int16_t j = 0;
	int j_diff = 0;
	...
	for (i=-R; i<=R; i++){
		width = sqrtf((int16_t)R_squared - i*i);
		j = -width;
		...
		for (j=-width; j<=width; j++){
			int16_t x = usC+j;
			int16_t y = usP+i;
			if (x<0 || y<0 || x>=LCD_Default_Max_COLUMN || y>=LCD_Default_Max_PAGE) continue;
			
			j_diff = my_abs(sqrtf((x-(int16_t)usC)*(x-(int16_t)usC) + (y-(int16_t)usP)*(y-(int16_t)usP)) - (int16_t)R);
			if (j_diff > 1) continue;
