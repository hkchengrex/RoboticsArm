...
/* ov7725 鲁隆脰脨露脧 路镁脦帽鲁脤脨貌 */
void OV7725_VSYNC_EXTI_INT_FUNCTION ( void )
{
    if ( EXTI_GetITStatus(OV7725_VSYNC_EXTI_LINE) != RESET ) 	//录矛虏茅EXTI_Line0脧脽脗路脡脧碌脛脰脨露脧脟毛脟贸脢脟路帽路垄脣脥碌陆脕脣NVIC 
    {
        if( Ov7725_vsync == 0 )
        {
            FIFO_WRST_L(); 	                      //脌颅碌脥脢鹿FIFO脨麓(脢媒戮脻from脡茫脧帽脥路)脰赂脮毛赂麓脦禄
            FIFO_WE_H();	                        //脌颅赂脽脢鹿FIFO脨麓脭脢脨铆
            
            Ov7725_vsync = 1;	   	
            FIFO_WE_H();                          //脢鹿FIFO脨麓脭脢脨铆
            FIFO_WRST_H();                        //脭脢脨铆脢鹿FIFO脨麓(脢媒戮脻from脡茫脧帽脥路)脰赂脮毛脭脣露炉
        }
        else if( Ov7725_vsync == 1 )
        {
            FIFO_WE_L();                          //脌颅碌脥脢鹿FIFO脨麓脭脻脥拢
            Ov7725_vsync = 2;
        }        
        EXTI_ClearITPendingBit(OV7725_VSYNC_EXTI_LINE);		    //脟氓鲁媒EXTI_Line0脧脽脗路鹿脪脝冒卤锚脰戮脦禄        
    }    
}
